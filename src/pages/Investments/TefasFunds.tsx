...
export function TefasFunds() {
  ...
  const tefasAssets = assets.filter((a) => a.asset_type === "TEFAS_FUND");
  ...
  return (
    <div className="flex flex-col h-full gap-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white">TEFAS Funds</h1>
          <p className="text-sm text-white/40 mt-0.5">{tefasAssets.length} assets</p>
        </div>
        <button
      ...
      <div className="flex-1 bg-[#0e0e18] border border-white/5 rounded-xl overflow-hidden flex flex-col">
        ...
        {rows.length === 0 ? (
          <div className="flex items-center justify-center flex-1">
            <p className="text-sm text-white/20">No TEFAS funds found</p>
          </div>
        ) : (
