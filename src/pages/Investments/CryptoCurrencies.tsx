...
const COLS = "1fr 100px 120px 120px 120px 120px";
  ...
  return (
    <div className="page-in" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <PageHeader
        ...
        meta={`${cryptoAssets.length} assets`}
        actions={
      ...
      <div className="surface" style={{ flex: 1, overflow: "hidden" }}>
        ...
        {rows.length === 0 ? (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 20px" }}>
            <p style={{ fontSize: 13, color: "var(--fg-5)" }}>No cryptocurrencies found</p>
          </div>
        ) : (
          rows.map(({ asset, holding, latestPrice, sym, currentValue, pnl, pnlPct }) => (
            ...
            >
              ...
              <span className="num" style={{ fontSize: 13, color: "var(--fg-2)" }}>{holding ? holding.quantity.toFixed(4) : "—"}</span>
      ...
      {showModal && (
        <TransactionModal mode="add" assets={allCryptoAssets} holdings={holdings} onSubmit={handleAddTransaction} onClose={() => setShowModal(false)} loading={mutating} />
      )}
    </div>
  );
}
