  ...
  return (
    <div className="bg-[#141422] border border-white/10 rounded-lg px-3 py-2 text-xs">
      ...
      {payload.map((p, i) => (
        <p
          key={i}
          className={p.name === "income" ? "text-emerald-400" : "text-red-400"}
        >
...
export function Dashboard() {
  ...
  const { transactions, categories, setTransactions, setCategories } =
    useAppStore();
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      await logError("Failed to load dashboard data", err);
